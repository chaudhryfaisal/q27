#!/usr/bin/env python3
"""Needle-in-haystack retrieval gate for the fp8-PV kernel (Q27_PF_PV8).

Reconstructs the documented fp8q needle protocol (docs/perf-attribution-
prefill-attn.md): a long filler haystack with 6 distinct
"calibration constant" needles planted at 10/35/60/70/78/95% depth; ask the
model to read back each. PASS = verbatim number at every depth. Deep prompts
are the ONLY place fp8-PV's extra P-quantization could silently corrupt
retrieval, so this is the default-on gate.

Server (separate terminal, GPU free):
  Q27_KV=fp8 Q27_PF_PV8=1 ./build/q27-server <model> <tok> --port 8081 \
    --ctx 200000 --no-think --fast-head
Then: python3 scratchpad/needle_pv8.py [--port 8081] [--depth 200000]
"""
import argparse
import json
import sys
